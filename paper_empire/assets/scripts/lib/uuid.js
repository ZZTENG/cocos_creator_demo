...
// UUID Object Component {{{
    ...
    UUID.parse = function (strId) {
        let r, p = /^\s*(urn:uuid:|\{)?([0-9a-f]{8})-([0-9a-f]{4})-([0-9a-f]{4})-([0-9a-f]{2})([0-9a-f]{2})-([0-9a-f]{12})(\})?\s*$/i;
        if (r = p.exec(strId)) {
            let l = r[1] || "", t = r[8] || "";
            if (((l + t) == "") ||
                (l == "{" && t == "}") ||
    ...
    UUID.prototype._init = function () {
        let names = UUID.FIELD_NAMES, sizes = UUID.FIELD_SIZES;
        let bin = UUID._binAligner, hex = UUID._hexAligner;
        ...
        for (let i = 0; i < 6; i++) {
            let intValue = parseInt(arguments[i] || 0);
...
// UUID Version 1 Component {{{
     ...
     */
    UUID.genV1 = function () {
        let now = new Date().getTime(), st = UUID._state;
        if (now != st.timestamp) {
            if (now < st.timestamp) {
                st.sequence++;
            }
        ...
        st.sequence &= 0x3FFF;
        let cshar = (st.sequence >>> 8) | 0x80; // set letiant '10'
...
// Misc. Component {{{
    ...
    /**
     * Preserves the value of 'UUID' global letiable set before the load of UUID.js.
     * @since 3.2
     * @type object
     */
