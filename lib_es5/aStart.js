...
var aStart = {
    //其中的MAP.arr是二维数组
    searchRoad: function searchRoad(start_x, start_y, end_x, end_y, rows, cols, arr) {
        var openList = [],
            //开启列表
        closeList = [],
            //关闭列表
        ...
        do {
            ...
            for (var i in surroundPoint) {
                ...
                item.y >= 0 && item.x < rows && item.y < cols && arr[item.x][item.y] == KeyValueManager['selfTeam'] && //判断是否是障碍物
                !this.existList(item, closeList) && //判断是否在关闭列表中
                arr[item.x][currentPoint.y] == KeyValueManager['selfTeam'] && //判断之间是否有障碍物，如果有障碍物是过不去的
                arr[currentPoint.x][item.y] == KeyValueManager['selfTeam']) {
                    //g 到父节点的位置
    ...
    //用F值对数组排序
    sortF: function sortF(a, b) {
        return b.F - a.F;
    },
    //获取周围八个点的值
    SurroundPoint: function SurroundPoint(curPoint) {
        var x = curPoint.x,
            y = curPoint.y;
        return [
        // {x:x-1,y:y-1},
    ...
    existList: function existList(point, list) {
        for (var i in list) {
