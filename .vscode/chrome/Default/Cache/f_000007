...
(function(global){
	var browser,//检测浏览器
		weburl='https://pay.17m3.com/gamepay/',//资源地址
		posturl='https://pay.17m3.com/sdk/CodeWappay.aspx',//ajax请求地址
		loadFiles,//加载外部文件方法
		...
		paystrJson={
			...
			'weixin':['weixin',weburl+'pay_weixin_qr.png','微信支付']},//显示的支付方式
	...
	*/
	...
	dhpayObj={
		payMethod: 'web',
		closecallback: function(){alert('test')},
		// finishCallBack: function(){ window.alert('param does not contain finishCallBack')},
		payfor:function(payObj){//payObj={postjson:用户信息,resultpost:false(订单处理结果请求是否生效),config:{alipay:true,weixin:true},closecallback:function}
			...
			var alipayStr='<li class="'+paystrJson['alipay'][0]+'"><a href="javascript:void(0);" data-type="'+paystrJson['alipay'][0]+'"><img src="'+paystrJson['alipay'][1]+'"><span>'+paystrJson['alipay'][2]+'</span></a></li>',
				weixinStr='<li class="'+paystrJson['weixin'][0]+'"><a href="javascript:void(0);" data-type="'+paystrJson['weixin'][0]+'"><img src="'+paystrJson['weixin'][1]+'"><span>'+paystrJson['weixin'][2]+'</span></a></li>';
				if(payObj.config){
					...
					paylistHtml=alipayStr+weixinStr;
				}
	...
	dhpayFn={
					...
					if(data.result==1){
						switch(_t.userData['payScene']){
							...
							case 2://手机第三方浏览器
								...
								}else if(_t.userData['payChannel']=='weixin'){ //移动H5+微信(星启天) =>微信WAPPAY接口 跳转
									if(interfaceObj!=null){
								...
								// else if(_t.userData['payChannel']=='alipayWap') {
								// 	if(interfaceObj!=null){
								// 		//使用sdk支付
								// 				postData.operate='requestOpenAPP_alipayPay';
								// 				postData.app='支付宝';
								...
								// 		}
								// 	}
								// } else if(_t.userData['payChannel']=='alipayWeb') {
								// 	if(parent !== window){//iframe内
								// 		postData.operate='requestOpenAPP_alipayPay';
								// 		postData.app='支付宝';
								// 		postData.resultInfo=_t.template(data.data,true,false);
								...
								// 		$('body').append(_t.template(data.data,true,false));
								// 	}
								// } else if(_t.userData['payChannel']=='weixinWap') {
								// 	if(interfaceObj!=null){
								// 		//使用sdk打开
								// 			postData.operate='requestOpenAPP_wxPay';
								// 			postData.app='微信';
								...
								// 		}
								// 	}
								// } else if(_t.userData['payChannel']=='weixinWeb') {
								// 	if(parent !== window){//iframe内
								// 		postData.operate='requestOpenAPP_wxPay';
								// 		postData.app='微信';
								// 		postData.resultInfo=data.data;
		...
		*/
		getpayBydata:function(){
			var _t=this;
			if((browser.versions.ios || browser.versions.android) && !browser.versions.weixin){//手机第三方浏览器 payScene=2
				_t.userData['payScene']=2;
			}else if((browser.versions.ios || browser.versions.android) && browser.versions.weixin){//微信浏览器(无支付宝) payScene=3
				if(_t.userData['payChannel']=='weixin' && typeof _t.userData['wxopenid']!='undefined' &&  _t.userData['wxopenid']!=''){//微信浏览器(必须具有公众号Openid)=>目前为闪电玩
